...
class MOBI6 {
    ...
    resolveHref(href) {
        ...
        if (!match) return { index: 0, anchor: () => null }
        const filepos = match[1]
        const number = Number(filepos)
        let index = this.#sections.findIndex(section => section.end > number)
        if (index === -1) index = Math.max(0, this.#sections.length - 1)
        const anchor = doc => doc ? (doc.getElementById(`filepos${filepos}`) || doc.getElementById(filepos)) : null
        return { index, anchor }
    }
    splitTOCHref(href) {
        const match = href.match(/filepos:(.*)/)
...
export async function loadMobiBook(fileOrBlob) {
    let blob = fileOrBlob
    if (blob instanceof ArrayBuffer || (typeof ArrayBuffer !== 'undefined' && ArrayBuffer.isView(blob))) {
        blob = new Blob([blob])
    }
    const mobi = new MOBI({
        unzlib: async (bytes) => {
            if (typeof DecompressionStream !== 'undefined') {
                try {
                    ...
                    console.warn('[MOBI] DecompressionStream error, using raw', e)
                }
            }
            return bytes
        }
